...
void {{ cookiecutter.repo_name }}::finter_writer_gzip::open(const char *filename) {
  if (!_gz_output) {
...
void {{ cookiecutter.repo_name }}::finter_writer_gzip::clear() {
  _good = true;
  _bad = _fail = _eof = false;
}

bool {{ cookiecutter.repo_name }}::finter_writer_gzip::is_open() const {
  return _gz_output;
}
