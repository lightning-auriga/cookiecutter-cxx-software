  ...
  if (!_gz_input) {
    ...
    if (!_gz_input)
      throw std::domain_error(
          "{{ cookiecutter.repo_name }}::finter_reader_gzip::open: could not "
          "open file \"" +
          std::string(filename) + "\"");
  } else {
    throw std::domain_error(
        "{{ cookiecutter.repo_name }}::finter_reader_gzip::open: reopen attempted "
        "on active handle");
  }
}
...
void {{ cookiecutter.repo_name }}::finter_reader_gzip::read(char *buf,
                                                        std::streamsize n) {
  int my_errno = 0;
  if ((my_errno = gzread(_gz_input, reinterpret_cast<void *>(buf), n)) < 0) {
    throw std::domain_error(
        "{{ cookiecutter.repo_name }}::finter_reader_gzip::read: read call of"
        " size " +
